...
for it = 1:length(LP_symbolic.names_x),
  ...
  my_x_indices = rba_model_indices.variables.(my_x_type).ids;
  my_x_n       = length(my_x_indices);
  n_x          = n_x + my_x_n;
  my_x_lb_symbolic = LP_symbolic.x_lb{it};
  my_x_ub_symbolic = LP_symbolic.x_ub{it};
  my_id_x      = rba_model_indices.variables.(my_x_type).ids;
  ...
  my_x_lb      = max([my_x_lb, rba_model_indices.variables.(my_x_type).LowerBound],[],2);
  my_x_ub      = min([my_x_ub, rba_model_indices.variables.(my_x_type).UpperBound],[],2);
  id_x         = [id_x; my_id_x];
  x_lb         = [x_lb; my_x_lb];
  x_ub         = [x_ub; my_x_ub];
end
...
for it = 1:length(LP_symbolic.names_a),
  ...
  switch my_rhs_string,
    ...
    otherwise
      ...
      my_a = rba_model_indices.parameters.(my_vector_string);
  end
  my_A_row = zeros(length(my_a),n_x);
  for itt = 1:length(LP_symbolic.names_x)
    my_x_type = LP_symbolic.names_x{itt};
    my_x_indices = rba_model_indices.variables.(my_x_type).indices;
    my_x_n = length(my_x_indices);
...
for it = 1:length(LP_symbolic.names_b),
  ...
  switch my_rhs_string,
    ...
    otherwise
      ...
      my_vector_string = my_rhs_string(3:end);
      my_b = rba_model_indices.parameters.(my_vector_string);
  end
