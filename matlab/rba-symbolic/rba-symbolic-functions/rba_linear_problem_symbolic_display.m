...
display('Matrix [A|a]')
[ {''}, rba_problem_symbolic.names_x', {''}; rba_problem_symbolic.names_a, rba_problem_symbolic.A, rba_problem_symbolic.a]

% display [B,b] as matrix

display('Matrix [B|b]')
[ {''}, rba_problem_symbolic.names_x', {''}; rba_problem_symbolic.names_b, rba_problem_symbolic.B, rba_problem_symbolic.b]
...
for it = 1:length(rba_problem_symbolic.a)
  display(rba_problem_symbolic.names_a{it})
  ind = find(cellfun('length',rba_problem_symbolic.A(it,:)));
  display(mytable([rba_problem_symbolic.A(it,ind)', repmat({'*'},length(ind),1), rba_problem_symbolic.names_x(ind)],0))
end

display(sprintf('Inequalities\n'));

for it = 1:length(rba_problem_symbolic.b)
  display(rba_problem_symbolic.names_b{it})
  ind = find(cellfun('length',rba_problem_symbolic.B(it,:)));
  display(mytable([rba_problem_symbolic.B(it,ind)', repmat({'*'},length(ind),1), rba_problem_symbolic.names_x(ind)],0))
end
